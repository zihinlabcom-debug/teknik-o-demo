...
if(process.argv.includes('--chat')) {
  const records=JSON.parse(await readFile('test-results/technical-research-live-v4.json','utf8'));
...
for(const identity of identities) {
 const events=[];
 try {const result=await researchManufacturer(identity,(raw,urls)=>{events.push({raw,urls});console.log(JSON.stringify({audit:identity,stage:raw.stage??'unknown',status:raw.status,verification:raw.verification,urls}));});results.push({identity,...result,events});}
 catch(error){results.push({identity,status:'error',message:error.message,events});}
 await writeFile('test-results/technical-research-live-v4.json',JSON.stringify(results,null,2));
}
