...
export function DiagnosticOutcome({ candidates, evidence = [] }: { candidates: Candidate[]; evidence?: string[] }) {
  return <section className="mt-3 rounded-2xl border border-slate-200 bg-white p-4" aria-label="Olası arızalar">
    ...
    {evidence.length > 0 && <div className="mb-3 rounded-xl bg-slate-50 p-3 text-xs text-slate-700"><strong>Değerlendirmede kullanılan gözlemler</strong>
      <ul className="mt-1 list-disc space-y-1 pl-4">{evidence.map((quote,index)=><li key={`${index}-${quote}`}>{quote}</li>)}</ul></div>}
